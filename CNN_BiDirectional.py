...
from keras.layers import Conv2D, MaxPooling2D, Dense, Flatten, Dropout, Input, Reshape, LSTM, Bidirectional, GRU
...
# LSTM Block
model.add(Reshape((180,128)))
model.add(LSTM(units=128, dropout=0.05, recurrent_dropout=0.35, return_sequences=True, input_shape=(180,128)))
model.add(Bidirectional(LSTM(units=128, dropout=0.05, recurrent_dropout=0.35, return_sequences=True)))
...
print('Fitting Model')
hist = model.fit(X_train, y_train, batch_size=128, epochs=120, validation_data=(X_valid, y_valid), shuffle=False)

## Rename File name ##
print('Saving hist model into cnn_blstm_s.pickle')
with open('cnn_blstm_s.pickle','wb') as f:
	pickle.dump(hist.history, f)

model.save('Model_CNN_BLSTM_S.h5')
##############################################
